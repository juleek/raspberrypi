...
TTempPollerWrapper::TTempPollerWrapper(TSensorInfo si) noexcept
    : TempPoller(si) {
   SensorInfo = si;
...
void TDriver::InitSignalHandlers() noexcept {
   ...
   connect(m_SigIntSocketNotifier.get(), &QSocketNotifier::activated, [this](int) { OnSigInt(); });
...
void TDriver::OnNewTemperatureGot(TTempPollerWrapper *Wrapper, QString ErrStr, double Temp) noexcept {
   qDebug().nospace() << "TDriver::OnNewTemperatureGot:"
                      ...
                      << ", MinPossibleTemp: " << Wrapper->SensorInfo.MinPossibleTemp << ", ErrStr: " << ErrStr
                      << ", T: " << Temp;
