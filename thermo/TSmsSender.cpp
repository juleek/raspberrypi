...
namespace {
void HandleFinishOfRequest(TCategory *Category, QNetworkReply *NetworkReply, QNetworkReply::NetworkError NetworkError) noexcept {
...
TSmsSender::TSmsSender(QString Login, QString Password, QString SenderId,
                       std::unordered_map<std::uint32_t, TCategoryInfo> Setup) noexcept: d_ptr(new TSmsSenderPrivate) {
   Q_D(TSmsSender);
   d->UrlTemplate = "http://smsc.ru/sys/send.php?"
                    "login="     + std::move(Login) +
                    "&psw="      + std::move(Password) +
